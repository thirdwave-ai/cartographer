...

  {
    ...
    for (const auto& submap_id_data : data_.submap_data) {
      ...
      if (submap_id_data.data.state == SubmapState::kFinished) {
        ...
        CHECK(traj_queue != k_nearest_by_trajectory.end());
        traj_queue->second.push(submap_id_data.id);
      }
    }
  }
  std::vector<int> ignored_trajectories;
  for (auto t : options_.ignored_trajectories()) {
    std::cerr << "Setting up to ignore trajectory " << t << std::endl;
    ignored_trajectories.push_back(t);
  }
...
void PoseGraph3D::HandleWorkQueue(
  ...
  for (auto constraint : result) {
    if (constraint.node_id.trajectory_id != constraint.submap_id.trajectory_id) {
      non_self_trajectory_connections = true;
      break;
    }
  }
